...
## 0. Requirements Grill Before Outlining

Before outlining or building slides, check whether the user's request is specific enough.

If key planning variables are missing, do not guess. Ask for them first.

This grill is mandatory unless the user's initial request already includes all major planning constraints.

Minimum variables to resolve:
- slide count or speaking time
- audience
- presentation purpose / decision needed
- required sections
- required source material
- template/style choice if multiple company styles are available; otherwise default to the official blank template

If the user does not know the structure, propose a section breakdown and page allocation, then ask for approval before building.

If the user already gave a full brief with slide count, audience, objective, structure, materials, and relevant template/style choice, skip the grill and proceed directly to outlining. If no alternate style is explicitly requested, assume the official blank template.

Example grill:
- How many slides should this be, or how long is the talk?
- Who is the audience?
- What is the exact objective: inform, compare options, persuade, get approval, defend research?
- What sections do you want included?
- Should I propose a slide allocation by section for you to approve first?

If the deck scope is still unclear after one round, ask one more compact clarification pass rather than generating a speculative deck.

If only one or two critical variables are missing, ask only those missing items instead of repeating the full grill.
...
If a slide design expects a visual exhibit, do not leave the slot empty. Either obtain a relevant figure / screenshot / web image that supports the point, or choose a different layout that does not require an image.
...
### When no figure is supplied
- First try to derive a useful figure from the user's source material.
- If that is not possible and the slide still benefits from a visual, obtain a relevant web-sourced image or screenshot.
- Prefer explanatory product screenshots, diagrams, maps, charts, interface captures, or other information-bearing visuals over decorative imagery.
- If no good visual is available, redesign the slide so it does not rely on an empty image placeholder.
...
If the user requires a corporate or institutional template to be preserved, apply this architecture inside the template's existing master/layout system. Do not rebuild the visual system from scratch unless the user explicitly asks.
...
- **This slide stays on screen during Q&A and should be the last substantive slide the audience sees.**
- Do NOT follow it with "Thank You" or a blank slide unless the user wants to preserve a company-branded closing-slide convention

Exception for branded company decks: if the user explicitly wants the corporate closing / contact slide preserved, keep Conclusions as the last substantive slide and place the branded closing slide after it.
...
| Ending on "Thank You" | End on conclusions as the last substantive slide; allow a branded closing slide only when the company style requires it |
