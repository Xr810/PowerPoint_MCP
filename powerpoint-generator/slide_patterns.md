...
## Before You Create Anything: Template-First Rule

If the user provides a branded `.pptx` template or asks you to preserve a corporate template, do not start from a blank deck and do not recreate the brand system manually. Instead:

1. Open the provided template
2. Reuse its existing master and named layouts
3. Replace placeholders only
4. Preserve logos, footer text, page-number treatment, and brand colours exactly

Use the code examples below only when creating from scratch or when adapting content inside a layout while still preserving the template's visual system.
